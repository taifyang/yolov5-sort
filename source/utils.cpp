...
{
	float gain = std::min(output_size.width * 1.0 / input_size.width, output_size.height * 1.0 / input_size.height);
	int pad_w = (output_size.width - input_size.width * gain) / 2;
	int pad_h = (output_size.height - input_size.height * gain) / 2;
	box.x -= pad_w;
	box.y -= pad_h;
	box.x /= gain;
	box.y /= gain;
